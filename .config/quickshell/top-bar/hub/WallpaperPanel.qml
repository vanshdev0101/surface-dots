...
Item {
    ...
    // Auto-derive a coordinated Hub/Bar/Background palette from the wallpaper:
    // find the dominant hue (weighted circular mean, ignoring near-gray noise),
    // then build background/hub-accent/bar-accent as related tones of that hue
    // instead of three unrelated flat colors.
    ...
    function hueSat(c) {
        ...
        var s = d === 0 ? 0 : (l > 0.5 ? d / (2 - mx - mn) : d / (mx + mn))
        var h = 0
        if (d !== 0) {
    ...
    function applyAutoAccent(pal) {
        if (!pal || pal.length === 0) return
        var sx = 0, sy = 0, wsum = 0, satSum = 0, satCount = 0
        for (var i = 0; i < pal.length; i++) {
            var hsl = hueSat(pal[i])
            if (hsl.s < 0.08) continue // skip near-gray/black/white samples, hue is noise there
            var rad = hsl.h * Math.PI / 180
            sx += Math.cos(rad) * hsl.s
            sy += Math.sin(rad) * hsl.s
            wsum += hsl.s
            satSum += hsl.s
            satCount++
        }
        // Fallback to a neutral green hue if the wallpaper is basically grayscale
        var dominantHue = wsum > 0 ? (Math.atan2(sy, sx) * 180 / Math.PI + 360) % 360 : 100
        var avgSat = satCount > 0 ? satSum / satCount : 0.35

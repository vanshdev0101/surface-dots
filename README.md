...
- **Added a keybinds cheat sheet** (`SUPER + /`) — a searchable overlay listing every configured shortcut. It's generated by parsing `hyprland.lua` directly (`keybinds-cheatsheet/parse_keybinds.py`), so it can't go stale the way a hand-written doc can — remap a key and the cheat sheet just reflects it next launch.
...
- `SUPER + /` → keybinds cheat sheet (searchable, generated live from `hyprland.lua`)
